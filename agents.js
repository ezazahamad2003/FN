...
- Shared settings today: the app's Shopify token now carries the navigation scope, so after Dan approves, the agent adds the store to the Mega Menu itself and the final check reads the live menu back to prove it (a checklist is only the fallback if that write fails); Shopify Flow has no API (checklist); Helium Customer Fields has no write API, so the agent reads the public registration form, reports whether the department tag is present and exactly where it belongs alphabetically, and Dan edits all three forms. With LOCKSMITH_ACCESS_TOKEN set the collection lock is created through the Locksmith Admin API, otherwise the agent hands over the lock checklist.
